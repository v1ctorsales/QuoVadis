...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 🔹 Criar Viagem (POST)
      ...
      // Converte os valores numéricos; se estiverem vazios, envia null
      const valorTransporteNum = valorTransporte ? parseFloat(valorTransporte) : null;
      const valorHospedagemNum = valorHospedagem ? parseFloat(valorHospedagem) : null;
      const valorGastoPasseiosNum = valorGastoPasseios ? parseFloat(valorGastoPasseios) : null;
      const valorGastoAlimentacaoNum = valorGastoAlimentacao ? parseFloat(valorGastoAlimentacao) : null;
      const valorOutrosGastosNum = valorOutrosGastos ? parseFloat(valorOutrosGastos) : null;
      const quantidadePessoasNum = quantidade_pessoas ? parseInt(quantidade_pessoas) : null;
      const precoSugeridoNum = preco_sugerido ? parseFloat(preco_sugerido) : null;
      const limiteParcelasNum = limite_parcelas ? parseInt(limite_parcelas) : null;
    ...
    // 🔹 Atualizar Viagem (PUT)
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          data_ida: dataIda,
          data_volta: dataVolta,
          quantidade_pessoas: quantidade_pessoas, // atualização do novo campo
          transporte: transporte,
          ...
          outros_gastos_por_pessoa: outrosGastosPorPessoa,
          limite_parcelas: limite_parcelas  // salva o valor convertido
        })
        .eq("id", id)
        .select();
